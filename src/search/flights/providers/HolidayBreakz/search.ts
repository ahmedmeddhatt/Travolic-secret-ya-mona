...
const { HOLIDAYBREAKZ_URL, HOLIDAYBREAKZ_PASSWORD } = process.env;
...
const search = function async(input: SearchOptions) {
  ...
  let URL = `${HOLIDAYBREAKZ_URL}?from=${options.legs[0].origin}&to=${legs[0].destination}&date1=${legs[0].departure}&adults=${options.adults}&children=${options.children}&infants=${options.infants}&partner=${partner}&password=${HOLIDAYBREAKZ_PASSWORD}&locate=${locate}&currency=${options.currency}&cabin=${options.cabinClass}&market=${market}`;
