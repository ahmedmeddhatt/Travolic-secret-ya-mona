...
const { CHEAPTICKETFARE_TOKEN, CHEAPTICKETFARE_URL } = process.env;
const cabinClassMap = {
  Economy: 'Y',
  Premium_Economy: 'S',
  Business: 'B',
...
const search = async (input: SearchOptions) => {
  ...
  let URL = `https://${domainCountry}${CHEAPTICKETFARE_URL}?tripType=${options.tripType}&origin1=${options.legs[0].origin}&destination1=${options.legs[0].destination}&departDate1=${departDate}&adults=${options.adults}&childs=${options.children}&infants=${options.infants}&classType=${options.cabin}&currency=${options.currency}&metaName=${meta_name}`;
