...
const providerName = 'UniJattFares';
const agent = {
  id: Buffer.from(providerName).toString('base64'),
  name: providerName
};

const filterSegemtData = (flight: any, airline: string) => {
...
const transformResults = (
  ...
  for (const offer of offers) {
    ...
    for (const seg of outboundLeg) {
      const transFormedSegment: TransformedSegmentWithoutID = filterSegemtData(
        seg,
        offer.marketingAirline
      );
      ...
      transformedResult.codeShare[
        ...
        operatingCarrier: offer.marketingAirline
      };
    }
    ...
    if (input.tripType === 'round') {
      ...
      for (const seg of inboundLeg) {
        const transFormedSegment: TransformedSegmentWithoutID =
          filterSegemtData(seg, offer.marketingAirline);
        const Segemnt = buildSegment(transFormedSegment);
        Segments.push(Segemnt);
        transformedResult.segments[Segemnt.id] = Segemnt;
        transformedResult.codeShare[
